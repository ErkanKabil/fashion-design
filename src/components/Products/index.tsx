    ...
    return (
        <>
            <div className={"flex justify-end gap-2 mb-4"}>
                <FilterDropdown items={uniqueSizes} title={"Size"} selectedItems={selectedSizes} handleChange={handleSizeFilter}/>
            ...
            <div className={"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3"}>
                {filteredProducts.map((product: IProduct) => <ProductCard product={product} key={product.id}/>)}
            </div>
        </>
)
};
